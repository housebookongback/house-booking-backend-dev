...
// Define regex patterns to find the method boundaries
const methodStartPattern = /\s+\/\/ Get single listing by ID \(simplified for frontend\)\n\s+getOneListing: async \(req, res\) => \{/;
const methodEndPattern = /\s+\},\n\n\s+\/\/ Get all listings for a specific host/;

// Find the positions of the method boundaries
const startMatch = content.match(methodStartPattern);
const endMatch = content.match(methodEndPattern);
...
const newContent = content.substring(0, startPos) + newMethod + content.substring(endPos);
